...
for i=1:size(col,1)
    if (row(i)>5)&&(row(i)<(size(pd_map_test,1)-5))&&(col(i)>5)&&(col(i)<(size(pd_map_test,2)-5))&&(mod(col(i),16)~=3)&&(mod(col(i),16)~=6)&&(mod(col(i),16)~=10)&&(mod(col(i),16)~=15)
        ...
        if(abs(out(row(i),col(i),3)-out(row(i),col(i),1))>=0.075)&&(out(row(i),col(i),3)>out(row(i),col(i),2))%不加后面这个条件会导致黄色斑加深加亮
            raw_test(row(i),col(i)) = raw_test(row(i),col(i))-0.04;
        end
...
figure,imshow(raw_test);
save('chart_fixed_intp_2',"raw_test");
out2 = Demosaic_Func(raw_test);
